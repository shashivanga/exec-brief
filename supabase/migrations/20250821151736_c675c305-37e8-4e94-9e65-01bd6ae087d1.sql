...
CREATE TABLE IF NOT EXISTS public.company_catalog (
  ...
  aliases TEXT[],
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
  UNIQUE(name, COALESCE(ticker, ''), COALESCE(domain, ''))
);
...
CREATE TABLE IF NOT EXISTS public.ingested_items (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now() NOT NULL,
  UNIQUE(org_id, COALESCE(competitor_id, '00000000-0000-0000-0000-000000000000'::uuid), COALESCE(industry_topic_id, '00000000-0000-0000-0000-000000000000'::uuid), source_kind, source_id)
);
...
DROP POLICY IF EXISTS "Org members can upload their org documents" ON storage.objects;
...
CREATE POLICY "Org members can view their org documents" ON storage.objects FOR SELECT USING (
  ...
  EXISTS (
    ...
    AND is_org_member(documents.org_id)
  )
);

CREATE POLICY "Org members can upload their org documents" ON storage.objects FOR INSERT WITH CHECK (
  bucket_id = 'uploads' AND auth.uid() IS NOT NULL
);

CREATE POLICY "Org members can update their org documents" ON storage.objects FOR UPDATE USING (
  bucket_id = 'uploads' AND 
  EXISTS (
    SELECT 1 FROM public.documents 
    WHERE documents.storage_path = storage.objects.name 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cards_org_dashboard_position ON public.cards (org_id, dashboard_id, position);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ingested_items_org_competitor_published ON public.ingested_items (org_id, competitor_id, published_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_kpi_values_org_kpi_period ON public.kpi_values (org_id, kpi_id, period DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_briefings_org_date ON public.briefings (org_id, for_date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ai_jobs_org_status_created ON public.ai_jobs (org_id, status, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_org_members_user_id ON public.org_members (user_id);

-- ============================================================================
-- 15) SEED DATA - CARD TEMPLATES
-- ============================================================================
